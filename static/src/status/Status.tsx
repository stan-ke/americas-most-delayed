...
function valueOf(
	...
	return (
		(s as unknown as Record<string, string | number | null>)[col.key] ?? null
	);
}
...
function Led(props: {
	source: SourceStatus;
	onHover: (s: SourceStatus | null) => void;
}) {
	...
	return (
		<Switch fallback={<td classList={{ num: props.col.num }}>{plain()}</td>}>
			<Match when={props.col.key === "status"}>
				<td
					class={`st ${stateClass(props.source)}`}
					title={statusText(props.source)}
				>
...
function Row(props: { source: SourceStatus; generatedAt: number }) {
	return (
		<tr>
			<For each={COLS}>
				{(c) => (
					<Cell source={props.source} col={c} generatedAt={props.generatedAt} />
				)}
			</For>
		</tr>
	);
	...
	const sorted = createMemo(() => {
		...
		return report
			...
			.sort((a, b) => {
				...
				if (col.num)
					return (
						dir * (((x as number) ?? -Infinity) - ((y as number) ?? -Infinity))
					);
				return (
					dir *
		...
		<>
            ...
            beside a frozen one — the counts would otherwise read as current. */}
				<Show
					...
					fallback={
						report.disconnected() ? "disconnected — retrying…" : "connecting…"
					}
				>
					{(s) => (
						<>
							<b>{s().total_sources}</b> sources · <b>{s().active}</b> active ·{" "}
							<b>{s().hot}</b> on leaderboard · <b>{s().requires_auth}</b> need
							auth · <b>{s().no_realtime}</b> no realtime ·{" "}
							<b>{s().no_vehicle_positions}</b> no vehicle pos ·{" "}
							<b>{s().failed}</b> failed · <b>{s().loading}</b> loading GTFS ·{" "}
							<b>{s().vehicles_now.toLocaleString()}</b> vehicles tracked now
						</>
					)}
				</Show>
			</div>
			...
			>
				<Show when={hovered()}>
					{(s) => tipText(s(), report.generatedAt())}
				</Show>
			</div>
			...
			<h2>
				All sources{" "}
				<span
					style={{ "font-weight": 400, color: "#888", "font-size": ".85rem" }}
				>
			...
			<table id="tbl">
				...
				<tbody>
					<For each={sorted()}>
						{(source) => (
							<Row source={source} generatedAt={report.generatedAt()} />
						)}
					</For>
				</tbody>
			</table>
