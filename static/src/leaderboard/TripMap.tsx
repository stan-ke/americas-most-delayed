...
export function TripMap(props: {
  ...
  const caption = () => {
    ...
    const where = endedAgo(e, props.now) === null ? "Vehicle at" : "Last seen at";
    return (
  ...
  const drawRoute = async (entry: LeaderboardEntry) => {
    ...
    try {
      const path =
        encodeURIComponent(entry.slug) + "/" + encodeURIComponent(entry.trip_id);
    ...
    <>
      ...
      <div id="nomap" classList={{ on: !placed() }}>
        <Show when={props.entry}>
          No live location for this vehicle — its agency's position feed hasn't placed
          it.
        </Show>
      </div>
