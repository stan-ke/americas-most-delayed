...
import { fmtDelay, peak, tripKey } from "../lib/format";
 ...
 */
const ROTATE_MS =
  Math.max(2, Number(new URLSearchParams(location.search).get("rotate")) || 12) * 1000;
...
export function Leaderboard() {
  ...
  createEffect(() => document.body.classList.toggle("empty", !selection.current()));
  createEffect(() =>
    document.body.classList.toggle("finished", selection.current()?.ended_at != null),
  );
  createEffect(() =>
    ...
    <>
      ...
      <footer>
        ...
        <button
          type="button"
          id="pause"
          title={rotation.paused() ? "Resume rotation (space)" : "Pause rotation (space)"}
