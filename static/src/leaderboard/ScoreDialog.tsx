    ...
    <dialog id="scorebox" ref={dialog} onClose={() => props.onClose()}>
      <div class="inner">
        <Show
          when={breakdown()}
          fallback={
            <p>No breakdown in this snapshot — is AMD_DEBUG set on the server?</p>
          }
        >
          {(b) => (
            <>
              <h3>
                #{props.entry!.rank} {props.entry!.agency} — route {props.entry!.route}
              </h3>
              <table>
                <tbody>
                  <For each={b().steps}>
              ...
              <p class="detail">
                Every factor multiplies the minutes late, so the board stays a delay
                ranking — the factors only reorder trips already in the same league. An
                unknown input scores 1.0 (neutral), never a penalty.
              </p>
            </>
          )}
        </Show>
